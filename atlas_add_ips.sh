...
payload=$(aws ec2 describe-instances --filters "Name=tag:owner,Values=$OWNERTAG" "Name=tag:Name,Values=$NAMETAG" "Name=instance-state-name,Values=running" | jq -r "[ .Reservations[].Instances[] | {\"ipAddress\":.PublicIpAddress, \"deleteAfterDate\":\"$expiry\"} ]")
